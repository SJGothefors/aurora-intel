...
import type { ExtractedReport, IntelCase, LlmStatus, SourceAssessment, VocabularyTerm } from '../../types';
...
export function IntakePanel({ llm, vocabulary, tagSuggestions, onCreate, onError }: IntakePanelProps) {
  ...
  const [sourceAssessment, setSourceAssessment] = useState<SourceAssessment>('Okänd');
    ...
    <div className="work-panel-content intake-panel">
      ...
      {mode === 'paste' ? (
        <>
          ...
          <label className="intake-source-assessment">
            <span>{t('form.sourceAssessment', 'Source assessment — set manually')}</span>
            <select value={sourceAssessment} onChange={(event) => setSourceAssessment(event.target.value as SourceAssessment)}>
              {(['Okänd', 'Låg', 'Medel', 'Hög'] as SourceAssessment[]).map((value) => <option key={value} value={value}>{value}</option>)}
            </select>
            <small>{t('intake.sourceAssessmentHint', 'This is the officer’s assessment and is never chosen by AI.')}</small>
          </label>
