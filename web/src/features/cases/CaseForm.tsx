...
import type { Actor, CaseStatus, IntelCase, SourceAssessment, VocabularyTerm } from '../../types';
...
const SOURCE_ASSESSMENTS: SourceAssessment[] = ['Okänd', 'Låg', 'Medel', 'Hög'];
  ...
  return (
    <form className="case-form" onSubmit={(event) => { event.preventDefault(); normalizePosition(); onSubmit(); }}>
      ...
      <div className="form-section">
        ...
        <fieldset className={uncertain('begrepp') ? 'term-checkboxes field-uncertain form-span-2' : 'term-checkboxes form-span-2'}>
          <legend>{t('form.vocabulary')}</legend>
          <small>{t('form.vocabularyCheckboxHint', 'Select every type that directly applies.')}</small>
          <div>{activeTerms.map((term) => {
            const checked = (value.begrepp ?? []).includes(term.name_sv);
            return <label className={checked ? 'is-checked' : ''} key={term.id}>
              <input type="checkbox" checked={checked} onChange={(event) => patch({ begrepp: event.target.checked ? [...(value.begrepp ?? []), term.name_sv] : (value.begrepp ?? []).filter((name) => name !== term.name_sv) })} />
              <span>{term.name_sv}</span>
            </label>;
          })}</div>
        </fieldset>
        <label>
        ...
        <label>
          <span>{t('form.sourceAssessment', 'Source assessment')}</span>
          <select value={value.source_assessment ?? 'Okänd'} onChange={(event) => patch({ source_assessment: event.target.value as SourceAssessment })}>
            {SOURCE_ASSESSMENTS.map((assessment) => <option key={assessment} value={assessment}>{assessment}</option>)}
          </select>
        </label>
        <label>
          <span>{t('form.activitySummary', 'Ledger activity (max 4 words)')}</span>
          <input value={value.activity_summary ?? ''} onChange={(event) => patch({ activity_summary: event.target.value.split(/\s+/).slice(0, 4).join(' ') || null })} />
        </label>
        <label>
          <span>{t('form.traitsSummary', 'Ledger traits (max 4 words)')}</span>
          <input value={value.traits_summary ?? ''} onChange={(event) => patch({ traits_summary: event.target.value.split(/\s+/).slice(0, 4).join(' ') || null })} />
        </label>
        <label className="form-span-2">
