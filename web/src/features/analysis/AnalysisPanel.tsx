...
export function AnalysisPanel({ caseCount, llm, job, questions, onRefresh }: Props) {
  ...
  return <section className="analysis-panel">
    <header><div><span>AUTOMATED INTELLIGENCE ASSESSMENT</span><h2>Assessment and collection priorities</h2></div><button type="button" disabled={caseCount < 3 || llm.status !== 'online' || job?.status === 'running'} onClick={onRefresh}>Refresh</button></header>
    {caseCount < 3 ? <div className="analysis-empty"><strong>At least 3 reports are required.</strong><p>The local model will update this view as the ledger grows.</p></div>
      : llm.status !== 'online' ? <div className="analysis-empty"><strong>Local model is stopped.</strong><p>Start AI mode to produce an assessment. Reports remain available offline.</p></div>
        : job?.status === 'pending' || job?.status === 'running' ? <div className="analysis-empty"><strong>Assessment in progress…</strong><p>The model is working only with local reports, weather and the knowledge bank.</p></div>
          : result ? <div className="analysis-content">
