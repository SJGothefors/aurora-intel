...
Settings: temperature `0.5`, up to five proposals. Run only when the count is greater than the configured threshold or when manually requested.

<!-- AURORA:A3:START -->
Uppgift: föreslå högst fem konkreta, observerbara spaningsfrågor som minskar informationsluckor i de bifogade ärendena.
...
Settings: temperature `0.4`; the backend performs FTS/date/geo/begrepp pre-filtering and supplies at most 40 rows.
