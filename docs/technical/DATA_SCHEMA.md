...
Local queue state: `id`, `type` (`extraction`, `questions`, `qa`, `assessment`, `overview`), `status` (`pending`, `running`, `done`, `failed`, `cancelled`), `created_at`, `started_at`, `finished_at`, `input_json`, `result_json`, `error_code`, `error_message`. `idx_ai_jobs_status_created` supports queue polling. Raw report text is not written to info-level logs. Queue rows are local user data and disappear with `data/`.
